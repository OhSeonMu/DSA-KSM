...
with open("/home/osm/test_1/LLC_output/total_" + args.ksm_type + "_" + args.workload_type + "_" + args.workload_size + ".csv", "r") as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        if(50 < float(row["Time"])) :
            times.append(float(row["Time"]))
            llc_miss_rates.append(float(row["LLC-total-access"]))
        
# Create the graph
plt.figure(figsize=(10, 6))
plt.plot(times, llc_miss_rates, marker='o', linestyle='-')
plt.title(args.ksm_type + "_" + args.workload_type + "_" + args.workload_size + ' LLC total access')
plt.xlabel('Time')
plt.ylabel('LLC total acces')
plt.grid(True)
plt.ylim(0,2000000)
...
plt.savefig("/home/osm/test_1/Graph/total_" + args.ksm_type + "_" + args.workload_type + "_" + args.workload_size + ".png")
plt.show()
