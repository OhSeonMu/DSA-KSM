...
with open("/home/osm/test_1/LLC_output/" + args.ksm_type + "_" + args.workload_type + "_" + args.workload_size + ".csv", "r") as csvfile:
    ...
    for row in reader:
        if(50 < float(row["Time"])) :
            ...
            llc_miss_rates.append(float(row["LLC-miss-rate"]))
        
# Create the graph
plt.figure(figsize=(10, 6))
plt.plot(times, llc_miss_rates, marker='o', linestyle='-')
plt.title(args.ksm_type + "_" + args.workload_type + "_" + args.workload_size + ' LLC miss rate')
plt.xlabel('Time')
...
plt.savefig("/home/osm/test_1/Graph/" + args.ksm_type + "_" + args.workload_type + "_" + args.workload_size + ".png")
plt.show()
